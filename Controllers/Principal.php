    ...
    public function listaProductos()
{
    ...
    if (!empty($json)) {
        foreach ($json as $producto) {
            ...
            $data['precio'] = $result['precio']; // sin formato
            $data['cantidad'] = $producto['cantidad'];
            $data['imagen'] = $result['imagen'];

            $subTotal = $data['precio'] * $producto['cantidad'];
            $data['subTotal'] = number_format($subTotal, 2, '.', ''); // sin símbolo S/
    ...
    $array['total'] = number_format($total, 2, '.', ''); // también sin S/
    ...
    public function syncCarrito()
{
    ...
    if (!empty($json)) {
        ...
        $res = array('msg' => 'Carrito sincronizado', 'status' => 'ok');
    } else {
        $res = array('msg' => 'Carrito vacío', 'status' => 'error');
    }
