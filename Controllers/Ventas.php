    ...
    public function buscarCliente()
{
    ...
    foreach ($data as $row) {
        $result['id'] = $row['id'];
        $result['label'] = $row['nombre'] . ' ' . $row['apellido']; // buscar por nombre + apellido
        $result['direccion'] = $row['direccion'];
        $result['correo'] = $row['correo']; // <-- NUEVO
        array_push($array, $result);
    }
    echo json_encode($array, JSON_UNESCAPED_UNICODE);
    die();
    ...
    public function registrar()
    {
       ...
       $fecha = date('Y-m-d H:i:s');
        ...
        $tipo = 2; // 2 = venta en caja (desde el admin)
        $venta = $this->model->registrarVenta($total, $fecha, $cliente, $pago, $this->id_usuario, $tipo);
