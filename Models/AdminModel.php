...
class AdminModel extends Query{
    ...
    public function getPedidos($tipo, $year)
    {
    $sql = "SELECT SUM(total) AS total FROM ventas WHERE YEAR(fecha) = $year AND tipo = $tipo AND estado = 1";
    return $this->select($sql);
    }
    ...
    {
        $sql = "SELECT SUM(total) AS total FROM ventas WHERE YEAR(fecha) = $year AND tipo = 2";
        return $this->select($sql);
    }
...
public function getVentas($anio)
{
    $sql = "SELECT SUM(total) AS total FROM ventas WHERE YEAR(fecha) = $anio AND tipo = 2 AND estado = 1";
    return $this->select($sql);
}



public function getVentasAnt($anio)
{
    $sql = "SELECT SUM(total) AS total FROM ventas WHERE YEAR(fecha) = $anio AND tipo = 2 AND estado = 1";
    return $this->select($sql);
}
