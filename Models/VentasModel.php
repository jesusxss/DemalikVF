...
{
    $sql = "SELECT id, nombre, apellido, direccion, correo 
            FROM usuarios 
            WHERE tipo = 2 AND estado = 1 
            AND (nombre LIKE '%$valor%' OR apellido LIKE '%$valor%') 
            LIMIT 10";
    return $this->selectAll($sql);
}

    public function registrarVenta($total, $fecha, $cliente, $pago, $idusuario, $tipo)
    {
    $sql = "INSERT INTO ventas (total, fecha, nombre, pago, tipo, id_usuario) VALUES (?,?,?,?,?,?)";
    return $this->insertar($sql, [$total, $fecha, $cliente, $pago, $tipo, $idusuario]);
    }
